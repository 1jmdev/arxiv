...
pub fn execute(arguments: Arguments) -> Result<String> {
    ...
    match arguments.command {
        ...
        Command::Section { id, section } => read(&client, ReadArguments {
            id,
        ...
        Command::Metadata { id, json } => {
            ...
            if json { json_output(&metadata) } else { Ok(metadata.display()) }
        }
        Command::Versions { id } => {
            ...
            Ok(metadata.versions.iter().map(|entry| {
        ...
        Command::References { id, ids } => {
            ...
            report_warnings(&document.warnings);
            if ids {
                Ok(document.reference_ids().iter().map(|id| format!("{id}\n")).collect())
            } else {
        ...
        Command::Search(arguments) => {
            ...
            if arguments.json {
                ...
                Ok(results.iter().map(|paper| format!("{}\n", paper.id)).collect())
            } else {
                Ok(results.iter().map(|paper| {
                    format!(
                        "{}  {}\n    {}\n    {}\n\n",
                        paper.id,
                        paper.title,
        ...
        Command::Source { id, files, file } => {
            ...
            if let Some(file) = file {
                ...
                Ok(entries.iter().map(|entry| format!("{}\n", entry.name)).collect())
            }
        }
...
fn read(client: &WebClient, arguments: ReadArguments) -> Result<String> {
    ...
    if arguments.chunks || arguments.chunk.is_some() {
        ...
        if arguments.chunks {
            let descriptions: Vec<_> = chunks.iter().enumerate().map(|(index, blocks)| {
                serde_json::json!({
                    "chunk": index + 1,
                    "characters": blocks.iter().map(|block| block.markdown.chars().count() + 2).sum::<usize>(),
                    "starts_with": blocks.first().map(|block| block.text.chars().take(80).collect::<String>()).unwrap_or_default(),
                })
            }).collect();
            if matches!(arguments.format, OutputFormat::Json) {
                return json_output(&descriptions);
            }
            return Ok(descriptions.iter().map(|chunk| {
                format!(
                    "{}  {} characters  {}\n",
                    chunk["chunk"],
                    chunk["characters"],
                    chunk["starts_with"].as_str().unwrap_or("")
                )
            }).collect());
        }
        let index = arguments.chunk.expect("chunk selection");
        ensure!(index > 0 && index <= chunks.len(), "chunk must be between 1 and {}", chunks.len());
...
fn download(client: &WebClient, id: &str, format: &str, destination: Option<&Path>) -> Result<String> {
    let metadata = metadata::load(client, &id.parse()?)?;
    let cached = paper::download(client, &metadata, format)?;
    let path = if let Some(destination) = destination {
        ensure!(!destination.exists(), "destination already exists: {}", destination.display());
